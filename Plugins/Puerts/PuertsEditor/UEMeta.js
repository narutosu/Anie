 ...
 */
var MetaSpecifier = /** @class */ (function () {
    /**
     * the constructor
     * @param specifier
     * @param values
     * @returns
     */
    function MetaSpecifier(specifier, values) {
        /**
     ...
     * @param metaData
     */
    MetaSpecifier.prototype.ApplyInMeta = function (metaData) {
     ...
     * @param metaData
     */
    MetaSpecifier.prototype.ApplyInIdentity = function (metaData) {
        if (this.Specifier == '') {
            return null;
        }
     ...
     */
    MetaSpecifier.prototype.IsMetaKey = function () {
     ...
     * @returns
     */
    MetaSpecifier.prototype.IsMetaKeyValue = function () {
     ...
     * @returns
     */
    MetaSpecifier.prototype.IsMetaKeyValues = function () {
        return this.Values != null;
    };
    /**
    ...
    MetaSpecifier.CommonMetaData = new Map([
        ["DisplayName", function (specifier, metaData) {
        ...
        ["FriendlyName", function (specifier, metaData) {
                if (specifier.IsMetaKeyValue()) {
        ...
        ["BlueprintInternalUseOnly", function (specifier, metaData) {
        ...
        ["BlueprintType", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
                    metaData.set("BlueprintType", 'true');
                    return true;
                }
        ...
        ["NotBlueprintType", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
        ...
        ["Blueprintable", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
        ...
        ["CallInEditor", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
        ...
        ["NotBlueprintable", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
        ...
        ["Category", function (specifier, metaData) {
                if (specifier.IsMetaKeyValue()) {
        ...
        ["Experimental", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
        ...
        ["EarlyAccessPreview", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
        ...
        ["DocumentationPolicy", function (specifier, metaData) {
                if (specifier.IsMetaKey()) {
        ...
        ["SparseClassDataType", function (specifier, metaData) {
                if (specifier.IsMetaKeyValue()) {
...
function extractMetaSpecifierFromExpression(expression, prefix, regExp) {
    var execRegExp = regExp == null ? new RegExp("^".concat(prefix, ".([A-Za-z]+)$")) : regExp;
    var execResult = execRegExp.exec(expression.getText().trim());
...
function extractMetaSpecifierFromBinaryExpression(expression, prefix, regExp) {
    var execRegExp = regExp == null ? new RegExp("^".concat(prefix, ".([A-Za-z]+)$")) : regExp;
    var execResult = execRegExp.exec(expression.left.getText().trim());
    if (execResult == null) {
        return null;
    }
    ...
    if (ts.isStringLiteral(expression.right)) { // specifier = value
        ...
        var bValid_1 = true;
        expression.right.elements.forEach(function (value) {
            if (!bValid_1) {
                return;
            }
...
function collectMetaDataFromIdentifyDecorator(expressions, prefix, specifiers, metaData, keyRegExp, keyValueRegExp) {
    var MetaKeyValueRegExp = keyValueRegExp == null ? new RegExp("^".concat(prefix, ".([A-Za-z]+)$")) : keyValueRegExp;
    var MetaKeyRegExp = keyRegExp == null ? new RegExp("^".concat(prefix, ".([A-Za-z]+)$")) : keyRegExp;
    expressions.forEach(function (value) {
        ...
        if (metaSpecifier == null) {
            console.warn("the ".concat(prefix, ": ").concat(value.getFullText(), " is not valid meta data"));
            return;
        }
        var applyResult = metaSpecifier.ApplyInIdentity(metaData);
        if (applyResult == null) {
            console.warn("the ".concat(prefix, ": ").concat(value.getFullText(), " is not valid meta data"));
        }
...
function collectMetaDataFromMetaDecorator(expressions, prefix, specifiers, metaData, keyRegExp, keyValueRegExp) {
    ...
    expressions.forEach(function (value) {
        ...
        if (metaSpecifier == null) {
            console.warn("the umeta: ".concat(value.getFullText(), " is not valid meta data"));
            return;
        }
        var applyResult = metaSpecifier.ApplyInMeta(metaData);
        if (applyResult == null) {
            console.warn("the umeta: ".concat(value.getFullText(), " is not valid meta data"));
        }
...
function collectMetaDataFromDecorator(decorator, prefix, specifiers, metaData) {
    ...
    var expressionText = expression.expression.getFullText(); //  get the callable signature
    //  should use cache to hold the reg exp object ?
    if (new RegExp("^".concat(prefix, ".").concat(prefix, "$")).test(expressionText)) { // the decorator match @prefix.prefix
        collectMetaDataFromIdentifyDecorator(expression.arguments, prefix, specifiers, metaData);
    }
    else if (new RegExp("^".concat(prefix, ".umeta$")).test(expressionText)) { // the decorator match @prefix.umeta
...
function getMetaDataFromDecorators(decorators, prefix) {
    ...
    decorators.forEach(function (value) {
...
function processClassMetaData(specifiers, metaData) {
    var bValidSpecifiers = true;
    var InvalidSpecifier;
    var bWantsPlacable = false;
    var ClassFlags = UE.ClassFlags.CLASS_None;
    var WithIn = "";
    var ConfigName = "";
    var ShowCategories = new Set();
    var HideCategories = new Set();
    var ShowSubCategories = new Set();
    var HideFunctions = new Set();
    var ShowFunctions = new Set();
    var AutoExpandCategories = new Set();
    var AutoCollapseCategories = new Set();
    var DontAutoCollapseCategories = new Set();
    var ClassGroupNames = new Set();
    var SparseClassDataTypes = new Set();
    /**
        ...
        switch (value.Specifier.toLowerCase()) {
            case 'NoExport'.toLowerCase():
                if (!value.IsMetaKey()) { // should be the meta key
                    return markInvalidSpecifier("".concat(value.Specifier));
                }
            ...
            case 'ShowCategories'.toLowerCase():
                ...
                value.Values.forEach(function (value) { ShowCategories.add(value); });
                break;
            case 'HideCategories'.toLowerCase():
                if (!value.IsMetaKeyValues()) {
                    return markInvalidSpecifier("".concat(value.Specifier));
                }
                value.Values.forEach(function (value) { HideCategories.add(value); });
                break;
            case 'ShowFunctions'.toLowerCase():
                if (!value.IsMetaKeyValues()) {
                    return markInvalidSpecifier("".concat(value.Specifier));
                }
                value.Values.forEach(function (value) { ShowFunctions.add(value); });
                break;
            case 'HideFunctions'.toLowerCase():
                if (!value.IsMetaKeyValues()) {
                    return markInvalidSpecifier("".concat(value.Specifier));
                }
                value.Values.forEach(function (value) { HideFunctions.add(value); });
                break;
            ...
            case 'ClassGroup'.toLowerCase():
                ...
                value.Values.forEach(function (value) { ClassGroupNames.add(value); });
                break;
            ...
            case 'ConversionRoot'.toLowerCase():
                ...
                break;
            default:
                markInvalidSpecifier("".concat(value.Specifier));
                break;
        }
    }
    /**
     ...
     *      function body
     */
    specifiers.forEach(function (value) {
        parseClassMetaSpecifier(value);
    });
    if (!bValidSpecifiers) {
        console.warn("invalid specifier for uclass: ".concat(InvalidSpecifier));
        return null;
    }
    var metaDataResult = new UE.PEClassMetaData();
    //  fill data to the class meta data structure
    metaDataResult.SetClassFlags(ClassFlags, bWantsPlacable);
    metaData.forEach(function (value, key) { metaDataResult.SetMetaData(key, value); });
    metaDataResult.SetClassWithIn(WithIn);
    metaDataResult.SetConfig(ConfigName);
    HideCategories.forEach(function (value) { metaDataResult.AddHideCategory(value); });
    ShowCategories.forEach(function (value) { metaDataResult.AddShowCategory(value); });
    ShowSubCategories.forEach(function (value) { metaDataResult.AddShowSubCategory(value); });
    HideFunctions.forEach(function (value) { metaDataResult.AddHideFunction(value); });
    ShowFunctions.forEach(function (value) { metaDataResult.AddShowFunction(value); });
    AutoExpandCategories.forEach(function (value) { metaDataResult.AddAutoExpandCategory(value); });
    AutoCollapseCategories.forEach(function (value) { metaDataResult.AddAutoCollapseCategory(value); });
    DontAutoCollapseCategories.forEach(function (value) { metaDataResult.AddDontAutoCollapseCategory(value); });
    ClassGroupNames.forEach(function (value) { metaDataResult.AddClassGroup(value); });
    SparseClassDataTypes.forEach(function (value) { metaDataResult.AddSparseDataType(value); });
...
function processFunctionMetaData(specifiers, metaData) {
    var bValidSpecifiers = true;
    var InvalidMessage;
    var FunctionFlags = BigInt(UE.FunctionFlags.FUNC_None);
    var FunctionExportFlags = 0n; // BigInt(UE.FunctionExportFlags.FUNCEXPORT_Final);
    var bSpecifiedUnreliable = false;
    var bSawPropertyAccessor = false;
    var bSealedEvent = false;
    var RPCId = 0;
    var RPCResponseId = 0;
    var EndpointName = '';
    var bForceBlueprintImpure = false;
    var CppValidationImplName = '';
    var CppImpName = '';
    var bAutomaticallyFinal = true;
    /**
    ...
    function parseNetServiceIdentifiers(InIdentifiers) {
        var IdTag = "Id";
        var ResponseIdTag = "ResponseId";
        var JSBridgePriTag = "Priority";
        var bResult = true;
        InIdentifiers.forEach(function (value) {
            if (!bResult) {
                return;
            }
            if (value.indexOf('=') != -1) { // a tag with an argument
                var TagAndArgument = value.split('=');
                if (TagAndArgument.length != 2) {
                    return markInvalidSince("Invalid format for net service identifers: ".concat(value));
                }
                var Argument = parseInt(TagAndArgument[1]);
                if (Number.isNaN(Argument) || Argument < 0 || Argument > (1 << 16)) {
                    return markInvalidSince("Invalid network identifier ".concat(value, " for function"));
                }
                ...
                if (EndpointName.length != 0) {
                    bResult = false;
                    return markInvalidSince("Function should not specify multiple endpoints - '".concat(value, "' found but already using '").concat(EndpointName, "'"));
                }
                EndpointName = value;
            }
        });
    ...
    function parseFunctionMetaSpecifier(value) {
        ...
        switch (value.Specifier.toLowerCase()) {
            ...
            case 'BlueprintImplementableEvent'.toLowerCase():
                if (!value.IsMetaKey()) {
                    return markInvalidSince("".concat(value.Specifier, " should be meta key"));
                }
                if (FunctionFlags & BigInt(UE.FunctionFlags.FUNC_Net)) {
            ...
            case 'ServiceRequest'.toLowerCase():
                if (!value.IsMetaKeyValues()) {
                    return markInvalidSince("".concat(value.Specifier, " should be meta values"));
                }
                if (FunctionFlags & BigInt(UE.FunctionFlags.FUNC_BlueprintEvent)) {
            ...
            case 'BlueprintPure'.toLowerCase():
                {
                    if (!value.IsMetaKey() && !value.IsMetaKeyValue()) {
                        return markInvalidSince("".concat(value.Specifier, " should be meta key or meta value"));
                    }
                    var bPure = true;
                    if (value.IsMetaKeyValue()) {
            ...
            case 'WithValidation'.toLowerCase():
                ...
                break;
            default:
                markInvalidSince("".concat(value.Specifier, " is not a valid specifier"));
                break;
        }
    }
    /**
    ...
    function validateFunctionFlags() {
        if (FunctionFlags & BigInt(UE.FunctionFlags.FUNC_Net)) {
            FunctionFlags |= BigInt(UE.FunctionFlags.FUNC_Event);
            var bIsNetService = !!(FunctionFlags & (BigInt(UE.FunctionFlags.FUNC_NetRequest) | BigInt(UE.FunctionFlags.FUNC_NetResponse)));
            var bIsNetReliable = !!(FunctionFlags & BigInt(UE.FunctionFlags.FUNC_NetReliable));
            //  replated function
            //      1. not static 
    ...
    if (!bValidSpecifiers) {
        console.warn("invalid meta data for ufunction: ".concat(InvalidMessage));
        return null;
    }
    var metaDataResult = new UE.PEFunctionMetaData();
...
function processParamMetaData(specifiers, metaData) {
    var bValidSpecifiers = true;
    var InvalidMessage;
    var PropertyFlags = BigInt(UE.PropertyFlags.CPF_None);
    /**
     * a helper function used to mark the meta data is invalid
     * @param reason
     */
    ...
    function parseParamMetaSpecifier(value) {
        ...
        switch (value.Specifier.toLowerCase()) {
            case 'Const'.toLowerCase():
                if (!value.IsMetaKey()) {
                    return markInvalidSince("".concat(value.Specifier, " should be a meta key"));
                }
    ...
    if (!bValidSpecifiers) {
        console.warn("invalid meta data for uparam: ".concat(InvalidMessage));
        return null;
    }
    var metaDataResult = new UE.PEParamMetaData();
...
function processPropertyMetaData(specifiers, metaData) {
    ...
    var InvalidMessage;
    var PropertyFlags = BigInt(UE.PropertyFlags.CPF_None);
    var ImpliedPropertyFlags = BigInt(UE.PropertyFlags.CPF_None);
    var bSeenEditSpecifier = false;
    var bSeenBlueprintWriteSpecifier = false;
    var bSeenBlueprintReadOnlySpecifier = false;
    var bSeenBlueprintGetterSpecifier = false;
    var RepCallbackName;
    /**
     * a helper function used to mark the meta data is invalid
     * @param reason
        ...
        switch (value.Specifier.toLowerCase()) {
            case 'EditAnywhere'.toLowerCase():
                ...
                if (bSeenEditSpecifier) {
                    return markInvalidSince("found more than one edit/visibility specifier ".concat(value.Specifier, ", only one is allowed"));
                }
            ...
            case "BlueprintSetter".toLowerCase():
                if (!value.IsMetaKeyValue()) {
                    return markInvalidSince("".concat(value.Specifier, " should be a meta key value"));
                }
                if (bSeenBlueprintReadOnlySpecifier) {
            ...
            case "BlueprintReadOnly".toLowerCase():
                ...
                if (bSeenBlueprintWriteSpecifier) {
                    return markInvalidSince("can not specify both BlueprintReadOnly and BlueprintReadWrite or BlueprintSetter for ".concat(value.Specifier));
                }
    ...
    function validatePropertyFlags() {
        ...
        var ParamFlags = BigInt(UE.PropertyFlags.CPF_Parm)
    ...
    if (!bValidSpecifiers) {
        console.warn("invalid meta data for uproperty: ".concat(InvalidMessage));
        return null;
    }
    var metaDataResult = new UE.PEPropertyMetaData();
    var FinalFlags = PropertyFlags | ImpliedPropertyFlags;
...
function compileClassMetaData(type) {
    ...
    var _a = getMetaDataFromDecorators(decorators, 'uclass'), specifiers = _a[0], metaData = _a[1];
...
function compileFunctionMetaData(func) {
    //  fetch the decorator
    var decorators = func.valueDeclaration != null ? func.valueDeclaration.decorators : null;
    if (decorators == null) { //  no decorators
        return null;
    }
    var _a = getMetaDataFromDecorators(decorators, 'ufunction'), specifiers = _a[0], metaData = _a[1];
...
function compileParamMetaData(param) {
    //  fetch the decorator
    var decorators = param.valueDeclaration != null ? param.valueDeclaration.decorators : null;
    if (decorators == null) {
        return null;
    }
    var _a = getMetaDataFromDecorators(decorators, 'uparam'), specifiers = _a[0], metaData = _a[1];
...
function compilePropertyMetaData(prop) {
    //  fetch the decorator
    var decorators = prop.valueDeclaration != null ? prop.valueDeclaration.decorators : null;
    if (decorators == null) {
        return null;
    }
    var _a = getMetaDataFromDecorators(decorators, 'uproperty'), specifiers = _a[0], metaData = _a[1];
